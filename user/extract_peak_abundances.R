...
##### Extracting peak/saturating biomass/fluorescence values:  ####
...
# extract extra traits for sat decay model specifically:
...
# Tips on dissecting model objects:
...
# these extract saturating/peak abundances from specific model types:
...
# This function receives a growth rate model object (grs) produced by 
# get.growth.rate() function, identifies which model is the selected 'best'
# model, accesses its parameters, and returns the corresponding peak/sat
# abundance values. NAs are returned for linear and lag models. 
# NOTE: values from gr.flr models should be interpreted with caution.
...
# example of applyign to single models:
